...
int _wcscmp(const wchar_t* s1, const wchar_t* s2) { // Own implementation of wcscmp
    while (*s1 && (*s1 == *s2)) {
        s1++, s2++;
    }
    return *s1 - *s2;
...
BOOL ReadFileWToByteArray(const WCHAR* szFileName, PBYTE* lpBuffer, PDWORD dwDataLen)
{
    BOOL bResult = FALSE;
    HANDLE hFile = NULL;
    ...
    if (GetFileSizeEx(hFile, &liFileSize)) 
    {
	    ...
	    {
		    *dwDataLen = (DWORD)fSize - 280; // sizeof(IV) + sizeof(encrypted AES key) + sizeof(original file size)
	    }
    } 
    else 
    {
